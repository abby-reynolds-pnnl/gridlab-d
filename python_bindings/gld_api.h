...
// Placeholder for Checkpoint modes
...
public:
    ...
    GLDErrorCode set_config_file(const std::string& config_file) {
        ...
        // This function can be used to set the path to the configuration file
    }


    // Load a GLM and return an error code
    GLDErrorCode load_glm(const std::string& filepath) {
        // Copy the file path to the member variable
        glm_file_path = filepath;

        // Call internal gridlab function to load the GLM
    }
    ...
    // Set the application mode (e.g., POWERFLOW, TIMESERIES, DELTAMODE)
	// This will be actually set in the application so don't really need this
	// unless the user explicitly wants to set the application type.
    ...
    private:
		...
		// Add more members as applicable
};
